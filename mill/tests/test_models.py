...
def test_lumber_str(log):
    lumber = make_lumber(log, count=4, thickness_mm=50, width_mm=100, length_mm=2000)
...
def test_lumber_volume_m3(log):
    ...
    lumber = make_lumber(log, count=2, thickness_mm=50, width_mm=100, length_mm=3000)
...
def test_log_lumber_volume_m3_sums_related(log):
    make_lumber(log, count=2, thickness_mm=50, width_mm=100, length_mm=2000)
    make_lumber(log, count=4, thickness_mm=25, width_mm=100, length_mm=2000)
...
def test_lumber_status_changed_at_set_on_create(db):
    before = timezone.now()
    lumber = Lumber.objects.create(thickness_mm=50, width_mm=100, length_mm=2000)
    assert before <= lumber.status_changed_at <= timezone.now()


def test_lumber_status_changed_at_updates_when_status_changes(db):
    lumber = Lumber.objects.create(thickness_mm=50, width_mm=100, length_mm=2000)
...
def test_lumber_status_changed_at_stable_when_other_fields_change(db):
    lumber = Lumber.objects.create(thickness_mm=50, width_mm=100, length_mm=2000)
    old_ts = timezone.now() - timedelta(days=5)
    Lumber.objects.filter(pk=lumber.pk).update(status_changed_at=old_ts)
...
def test_lumber_days_in_status(db):
    lumber = Lumber.objects.create(thickness_mm=50, width_mm=100, length_mm=2000)
...
def test_suggested_price_sek_ex_vat_at_base_dim(db):
    # 45×95×1000 mm at base dim → 46 SEK inc VAT per metre; ex VAT = 46/1.25 = 36.80
    lumber = Lumber(thickness_mm=45, width_mm=95, length_mm=1000)
    assert lumber.suggested_price_sek == Decimal("36.80")


def test_suggested_price_sek_scales_with_dim_and_length(db):
    # 50×100×3000 mm — inc VAT = 46/(45*95) * 50*100*3 ≈ 161.40 → ex VAT ≈ 129.12
    lumber = Lumber(thickness_mm=50, width_mm=100, length_mm=3000)
...
def test_is_sold_false_when_no_price(log):
    lumber = make_lumber(log, count=2, thickness_mm=50, width_mm=100, length_mm=2000)
...
def test_count_sums_across_sources(db, species):
    log_a = Log.objects.create(species=species, diameter_cm=20, length_cm=200, mill_date=date(2026, 5, 1))
    log_b = Log.objects.create(species=species, diameter_cm=25, length_cm=200, mill_date=date(2026, 5, 1))
    lumber = make_lumber(
        sources=[(log_a, 2), (log_b, 3)],
        thickness_mm=50, width_mm=100, length_mm=2000,
    )
    assert lumber.count == 5


def test_volume_m3_uses_total_count_across_sources(db, species):
    log_a = Log.objects.create(species=species, diameter_cm=20, length_cm=200, mill_date=date(2026, 5, 1))
    log_b = Log.objects.create(species=species, diameter_cm=25, length_cm=200, mill_date=date(2026, 5, 1))
    lumber = make_lumber(
        sources=[(log_a, 2), (log_b, 3)],
    ...
    # unit 50*100*2000/1e9 = 0.010, * 5 = 0.050
    assert lumber.volume_m3 == pytest.approx(0.050)


def test_species_label_dedupes_and_sorts(db):
    tall = Species.objects.create(name="Tall")
    gran = Species.objects.create(name="Gran")
    log_t = Log.objects.create(species=tall, diameter_cm=20, length_cm=200, mill_date=date(2026, 5, 1))
    log_g = Log.objects.create(species=gran, diameter_cm=25, length_cm=200, mill_date=date(2026, 5, 1))
    log_t2 = Log.objects.create(species=tall, diameter_cm=22, length_cm=200, mill_date=date(2026, 5, 1))
    lumber = make_lumber(
        sources=[(log_t, 1), (log_g, 1), (log_t2, 1)],
        thickness_mm=50, width_mm=100, length_mm=2000,
    )
    assert lumber.species_label == "Gran / Tall"


def test_lumber_source_volume_and_revenue(log):
    lumber = make_lumber(
        log, count=4, thickness_mm=50, width_mm=100, length_mm=2000,
        unit_price_sek=Decimal("100.00"),
    )
    source = lumber.sources.get()
    # 50*100*2000/1e9 = 0.010 per board, * 4 = 0.040
    assert source.volume_m3 == pytest.approx(0.040)
    assert source.revenue_sek == Decimal("400.00")


def test_lumber_source_revenue_none_when_unsold(log):
    lumber = make_lumber(log, count=4, thickness_mm=50, width_mm=100, length_mm=2000)
    assert lumber.sources.get().revenue_sek is None
